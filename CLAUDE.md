...
**Figure encoding: colour = tokenizer, texture = serving SKU.** One hue per distinct tokenizer (`figures._SLOT_BY_FLAGSHIP`), inherited by every model folded into it; models that share a tokenizer but differ in price are separated by hatch (`_HATCH_BY_COUNTER`), never by hue. So a colour means the same thing in every figure, and the dollar charts *show* the "same tokens, different price" point instead of merely captioning it. `_series_style` raises if two counters in one chart resolve to the same (colour, hatch). Palette provenance and the CVD/ΔE gates are documented at `_SLOT_BY_FLAGSHIP`; the validator used is external and not committed, so re-validate with an equivalent check before changing a hue.
