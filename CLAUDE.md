...
Separately, **one FLORES sentence contains HTML** — `vie_Latn` index 429, `km<sup>2</sup>` — and it is the only line carrying markup in any of the ten corpus files (6 tags in ~260K characters). Every counter tokenizes it as content, which is faithful to upstream FLORES and immaterial to every headline number. It is worth knowing only because it was the single sentence on which the two Gemini tokenizer artifacts disagreed, and it briefly looked like a language finding.
